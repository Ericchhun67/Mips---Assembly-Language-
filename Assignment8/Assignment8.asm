...
#
# part1
# Using Recursive fuction to a stack frame and implement the fibonacci fuction,
# with all parameters passing to a stack
#
        ...
        .data
# prompt for user and get results
prompt: .asciiz " Enter an integer to find the fibonacci number : "
res:    .asciiz " Fibonacci is "
equalto: .asciiz " ) = "
...
main:	
	jal	getNum			# Get an integer from user 
	nop
	addiu	$sp, $sp, -4 
	...
	syscall
	la	$a0, equalto		# print the string
	li	$v0, 4
	syscall
	lw	$t0, 4($sp)
...
getNum:	
	la	$a0, prompt		# load and print the prompt
	li	$v0, 4
	syscall
	li	$v0, 5			# get the user data
	syscall 
	jr	$ra			# then exit subroutine
	nop
...
output:	
	lw	$t0, 4($fp)		# load the fib(n-1) to $t0
	nop
