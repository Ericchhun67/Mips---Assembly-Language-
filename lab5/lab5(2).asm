...
filter: 
	bge	$t2, $t3, done	# branch to done when index is 11
	nop			# branch delay
	...
	lw	$t5, ($t0)	# load last element of array into $t5
	nop
	sw	$t5, ($t1)	# store last element of array to result
	nop

	# Printing the arrays
	la	$a0, arr	#Print message
	li	$v0, 4
	syscall
